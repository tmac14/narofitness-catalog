...
- **Catalog UI agent** owns catalogue visual polish after `INTEGRATED` (step 11).
- **App UX agent** owns app-wide plain-language UX on affected pages (not `api.ts`).
...
| API-5 | Bulk reorder (catalog line items) | Agent 2 | Agent 4 | P1 | **INTEGRATED** | No — Catalog UI agent DnD smoke pending |
| API-6 | Presentation master reorder | Agent 2 | Agent 4 → Catalog UI agent visual | P3 | OPEN | No |
| API-7 | Category tree reorder | Agent 2 | Agent 4 → Catalog UI agent visual | P3 | OPEN | No |
| API-8 | Bulk layout skipped reasons | Agent 2 | Agent 4 → Catalog UI agent visual | P1 | **INTEGRATED** | Partial — bulk feedback manual QA |
...
**Agent 2 validated (2026-06-07):** API-1 `CONFIRMED` → `QA_READY`; API-3, API-5, API-8 `CONFIRMED`. See `docs/product/planning/contracts/API-*_CONTRACT_SUMMARY.md`.
...
**Next work:** Catalog UI agent **API-5 DnD smoke** on QA Stress Catalog; optional polish; then decide **API-4** vs catalogue performance/polish.
...
**Transversal (separate track):** [TRANSVERSAL_BACKLOG.md](./TRANSVERSAL_BACKLOG.md) · [planning state.md](./planning state.md).
...
| Field | Value |
|-------|-------|
| **Owner** | Agent 2 (backend); App UX agent (frontend); Agent 6 (PDF) |
...
### Frontend (App UX agent — QA_PENDING)
...
| Field | Value |
|-------|-------|
| **Owner** | Agent 2 (backend); Agent 4 (`api.ts`); App UX agent (UI) |
...
**App UX agent (post-INTEGRATED, optional):** labels, a11y, plain-language copy on `ProductsPage` — not data wiring.

**Catalog UI agent:** no role on `ProductsPage`.
...
- ProductsPage labels/accessibility polish belongs to App UX agent
...
| **Blocks manual QA** | Catalog UI agent DnD smoke on stress catalog pending |
...
**Catalog UI agent next:** Manual DnD smoke on QA Stress Catalog; optional toast/loading polish on "Guardar orden".
...
**Catalog UI agent (post-INTEGRATED):** visual polish for skip-reason list in Presentation Builder if needed.
...
- [x] Frontend integration complete; Catalog UI agent may polish display
...
- Visual polish for skip-reason list belongs to Catalog UI agent
...
Tests: [commands]
Agent 4: wire reorder API first; Catalog UI agent: visual DnD UI after INTEGRATED
```
...
Endpoint: [method path]
Tests: [commands]
Agent 4: wire category reorder API first; Catalog UI agent: visual DnD after INTEGRATED
```
...
Known reasons: [list]
Tests: [commands]
Agent 4: CatalogPresentationBuilder skip-reason wiring; Catalog UI agent: visual polish
```
...
| Handoff to Catalog UI agent | Components Catalog UI agent may visually polish after `INTEGRATED` |

### Per-dependency Agent 4 scope (after CONFIRMED)

| ID | Agent 4 integrates | Catalog UI agent polishes after INTEGRATED |
|----|-------------------|-----------------------------------|
| API-1 | — (seeder only) | — |
| API-3 | ~~`listMasters` pagination, `ProductsPage.tsx`~~ **INTEGRATED** | Labels, a11y, pager UX clarity (App UX agent) — Catalog UI agent: no role |
| API-4 | `getCatalog` / `getCatalogLayoutStatus` pagination, builder pages | Pagination bar styling |
| API-5 | ~~Reorder endpoint in `api.ts`, `catalogLines.ts`~~ **INTEGRATED** | Save/discard UX, toasts (Catalog UI agent polish) |
| API-6 | Master reorder API wiring | Presentation DnD visuals |
| API-7 | Category reorder API wiring | Category DnD visuals |
| API-8 | ~~Skip-reason list in `CatalogPresentationBuilder`~~ **INTEGRATED** | Bulk feedback banner visual polish (Catalog UI agent) |
