...
Canonical registry of API contracts between the desktop app and the backend. **Agent 4 integrates** after Agent 2 Contract Summary and `CONFIRMED`. **Catalog UI agent** polishes catalogue UI after `INTEGRATED`. **App UX agent** may improve plain-language labels on affected app-wide pages (not `api.ts`).

**Last updated:** 2026-06-11 (Codex — source-catalog dual-path priority)

**Frontend agent split:** Catalogue contract polish → Catalog UI agent. App-wide page copy/UX around integrated endpoints → App UX agent (coordinate shared files via Agent 3).
...
|--------|---------|----------|
| `PROPOSED` | Catalog UI agent identified need; Agent 3 drafted shape | Agent 3 records API-X |
...
| `INTEGRATED` | Agent 4 shipped contract wiring; tests pass | Catalog UI agent catalogue polish; App UX agent app-wide label UX if applicable |
| `QA_READY` | Backend confirmed; no frontend integration required | User / Catalog UI agent manual QA |

**File lock during `CONFIRMED` → `INTEGRATED`:** Agent 4 owns `api.ts` and related types. Catalog UI agent and App UX agent must not edit those files.
...
| Catalog UI agent | Optional export modal polish — not `api.ts` |
...
| Catalog UI agent | Optional polish only if QA finds visual issues |
...
### Frontend consumers (App UX agent)
...
Per-dependency guidance split by role. **Catalog UI agent** = catalogue visual/UI. **App UX agent** = app-wide plain-language UX (not `api.ts`). **Agent 4** = contract integration after `CONFIRMED`.

---

## API-1 — QA/stress seeder

### Catalog UI agent (catalogue visual / QA)
...
| **No role on ProductsPage** | Catalog UI agent does not polish `ProductsPage` — see App UX agent below |
...
### App UX agent (app-wide UX — post-INTEGRATED, optional)
...
| **Blocking?** | No — not required before Catalog UI agent Phase 1 QA unless coordination decides otherwise |
...
| **Do NOT** | Build design-heavy DnD UI — Catalog UI agent owns visual layer after integration |
...
| **Do NOT** | Design-heavy category tree UI — Catalog UI agent polishes after |
...
| **Catalogue-only** | Catalog UI agent scope — not app-wide |
