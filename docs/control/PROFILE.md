# Narofitness project profile

Profile: narofitness

Narofitness is a **PIM/catalog application** that optionally adopts **[Ordia](../ordia/README.md)** for durable agent orchestration (**Ordia v0.9**; baseline [SPEC v0.8](../ordia/SPEC_v0.8.md)). Manifest: [`ordia.yaml`](../../ordia.yaml).

Product decisions: `ORDIA-D001`–`ORDIA-D023` in `docs/control/DECISION_LOG.md`. Active plan: [IMPROVEMENT_PLAN_v0.8.md](../ordia/IMPROVEMENT_PLAN_v0.8.md) (**CLOSED**).

Validate: `npm run ordia:validate` · `npm run ordia:validate -- --project` · `npm run control:validate`.

**Workflow intents (ORDIA-D023):** `npm run ordia:prompt -- emit --intent <ID> --task <TASK-ID>` (list: `npm run ordia:workflow:list`). See [DAILY_USAGE.md](../ordia/DAILY_USAGE.md).
...
1. `docs/control/ORCHESTRATION_STATE.md` — live orchestration state
2. `docs/control/CONTROL_PLANE_RECOVERY_RUNBOOK.md` — context-loss recovery and active-task bootstrap
3. `docs/control/TASK_EXECUTION_PROTOCOL.md` — universal planning, dependency, lock, and validation gates
4. `docs/control/TASK_REGISTRY.yaml` — authoritative task, dependency, lock, and parallel-safety registry
5. `docs/control/AGENT_REGISTRY.yaml` — authoritative agent capability and topology registry
6. The active task packet under `docs/control/tasks/`, when one exists
...
9. `docs/control/COMMANDS.md` — canonical npm commands (required before QA, audit, validation, or environment setup prompts)

For a quick terminal index use `npm run help` or `npm run help -- <command>`; `docs/control/COMMANDS.md` remains the canonical reference for prompts and stop conditions.
...
When citing shell commands in prompts or stop conditions, use **only** commands from `docs/control/COMMANDS.md`.
...
this profile and the control protocols:
...
| `ordia-coordination-docs.mdc` | Editing control documentation |
...
- the active task packet under `docs/control/tasks/`;
- `docs/control/EVIDENCE_INDEX.md` when durable evidence changes;
- `docs/control/DECISION_LOG.md` only to record an explicit user decision.
...
Use `docs/control/AGENT_REGISTRY.yaml` as the authoritative role source.
...
5. Required regressions and proof steps (commands from `docs/control/COMMANDS.md` when applicable).
