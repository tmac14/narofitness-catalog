# Documentation inventory

Top-level coordination and adoption docs. Update when adding/removing `.md`/`.yaml` under control root.

## Coordination docs

- `docs/control/NAVIGATION.md`
- `docs/control/TASK_REGISTRY.yaml`
- `docs/control/AGENT_REGISTRY.yaml`
- `docs/control/MODEL_REGISTRY.yaml`
- `docs/control/ORCHESTRATION_STATE.md`
- `docs/control/PROFILE.md`
- `docs/control/DECISION_LOG.md`
- `docs/control/EVIDENCE_INDEX.md`
- `docs/control/ADOPTION_REPORT.md`
- `docs/control/commands.catalog.json`
- `docs/control/COMMANDS.md`
- `docs/control/DOCUMENTATION_INVENTORY.md`
- `docs/control/DOCUMENTATION_INVENTORY.yaml`
- `docs/control/adoption.checklist.yaml`

## Protocols

- `docs/control/protocols/CURSOR_ORCHESTRATION.md`
- `docs/control/protocols/CURSOR_IMPLEMENTATION.md`
- `docs/control/protocols/CODEX_ORCHESTRATION.md`
- `docs/control/protocols/CODEX_IMPLEMENTATION.md`
- `docs/control/protocols/TASK_EXECUTION.md`
- `docs/control/protocols/RECOVERY_RUNBOOK.md`
- `docs/control/protocols/RUNTIME_HANDOFF.md`

## Task packets

- `docs/control/tasks/TASK_PACKET_TEMPLATE.md`

## Workflows

- `docs/control/workflows/intents.overlay.yaml`

## Repository docs (sample)

- `COMMANDS.md`
- `docs/API.md`
- `docs/archive/product/es/ANALISIS_FUNCIONAL.md`
- `docs/archive/product/es/ARQUITECTURA_TECNICA.md`
- `docs/archive/product/es/README.md`
- `docs/archive/README.md`
- `docs/BUILDER_BACKEND_DEPENDENCIES.md`
- `docs/CATALOG_LAYOUTS.md`
- `docs/design/NAROFITNESS_DESIGN_SYSTEM_v2.md`
- `docs/DOCUMENTATION_GOVERNANCE.md`
- `docs/ENGINEERING_STANDARDS.md`
- `docs/MANUAL_QA_APP_WIDE.md`
- `docs/MANUAL_QA_BUILDER_UI.md`
- `docs/MANUAL_QA_PRESENTATION_BUILDER.md`
- `docs/product/FUNCTIONAL_ANALYSIS.md`
- `docs/product/planning/API_DEPENDENCY_BACKLOG.md`
- `docs/product/planning/APP_PLATFORM_UX_3.0_ROADMAP.md`
- `docs/product/planning/APP_WIDE_UX_SCOPE.md`
- `docs/product/planning/CATALOG_PRESENTATION_BACKLOG.md`
- `docs/product/planning/contracts/API-1_CONTRACT_SUMMARY.md`
- `docs/product/planning/contracts/API-1_VALIDATION_CHECKLIST.md`
- `docs/product/planning/contracts/API-3_CONTRACT_SUMMARY.md`
- `docs/product/planning/contracts/API-5_CONTRACT_SUMMARY.md`
- `docs/product/planning/contracts/API-8_CONTRACT_SUMMARY.md`
- `docs/product/planning/contracts/APP_JOBS_CONTRACT.md`
