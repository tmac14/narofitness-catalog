# App-Wide UX Scope (Agent 1B)

Discovery and planning reference for **Agent 1B — App-Wide Accessible UX/UI Agent**. Defines what is outside the catalogue builder (Agent 1A) and how Agent 1B should begin work.

**Last updated:** 2026-06-12 (Agent 3 — APP-PLATFORM-UX-3.0 Phase 1 **VALIDATED**)

**Owner:** Agent 1B. Agent 3 tracks shared-file locks.
...
| **QA** | Agent 1A — **`UX30_PHASE0_QA_PASS_WITH_NOTES`** (351/351; build OK) |
...
## Agent 1B implementation record (iterations 1–5)

**Recorded by:** Agent 3 (from Agent 1B completion report)
...
### Scope confirmation (Agent 1B did not touch)
...
These files were edited under Agent 1B iterations 1–5. Recorded as **completed SHARED work**; **no active locks** remain for this pass.
...
## Agent 1B implementation record (Iteration 6 — SHARED-2)
...
**Lock:** `SHARED-2` — completed by Agent 1B; validated by Agent 1A static smoke; **no active lock**.

---

## Agent 1A catalogue smoke result (Phase 1B-A — post Iteration 6)

**Recorded by:** Agent 3 (from Agent 1A audit report)
...
| SHARED-2 | May remain **released** from Agent 1A perspective |

**Manual visual checks still recommended (non-blocking):**

- Outline button appearance in builder/export flows
- PDF export health badge wording in `Layout` (engine name when active) — Agent 6 owns semantics; Agent 1B may polish copy via shared-file lock
...
- Pagination empty edge case — not blocking per Agent 1A audit

---

## Agent 1A catalogue smoke checklist (reference — completed static pass)
...
Agent 1B must **not implement** until the user approves a written plan.
...
| 4 | Identify shared files that overlap Agent 1A | Shared-file coordination requests |
...
### Deliverable template (Agent 1B → Agent 3 → User)
...
Blocked by Agent 1A/4 locks: [list]
...
|-------|------|-------|-------|
| `/` | `Dashboard.tsx` | **Agent 1B** | Home / entry point |
| `/suppliers` | `SuppliersPage.tsx` | **Agent 1B** | |
| `/import` | `ImportPage.tsx` | **Agent 1B** | + `components/import/**` |
| `/products` | `ProductsPage.tsx` | **Agent 1B** (UX/a11y) + **Agent 4** (API-3 data) | Agent 1A has **no role** on this page |
| `/products/:id` | `ProductDetailPage.tsx` | **Agent 1B** | |
| `/categories` | `CategoriesPage.tsx` | **Agent 1B** | |
| `/catalogs` | `CatalogsPage.tsx` | **Agent 1A** | Catalogue list |
| `/catalogs/:id` | `CatalogEditorPage.tsx` | **Agent 1A** | Builder + PreviewWorkspace |
| `/price-lists` | `PriceListsPage.tsx` | **Agent 1B** | |
| `/settings` | `SettingsPage.tsx` | **Agent 1B** | |

**Catalogue-only components (Agent 1A):** `components/catalog-builder/**`

**Import flow (Agent 1B):** `components/import/**`

---

## Agent 1A vs Agent 1B boundaries

| Area | Agent 1A (Catalogue) | Agent 1B (App-wide) |
|------|------------------------|---------------------|
...
**Rule:** Agent 1B does **not** edit catalogue builder files unless Agent 3 documents an explicit handoff from Agent 1A.

**Rule:** Agent 1A does **not** edit app-wide pages or navigation unless explicitly assigned.

**Rule:** Agent 1A has **no role** on `ProductsPage` unless user reclassifies it as catalogue workflow.

---

## Shared primitives and shared files (closed rules)

Agent 1B is the **default accessibility steward** for shared UI primitives (`components/ui/**`), but this is **not unrestricted ownership**.
...
| Agent | Rule |
|-------|------|
| **Agent 1B** | May propose app-wide a11y improvements to shared primitives — **lock required first** |
| **Agent 1A** | Must request lock to modify shared primitives for catalogue needs |
| **Agent 1A (preferred)** | Catalogue-specific wrappers/variants live under `components/catalog-builder/**` — no shared lock unless touching shared imports |

| File / area | Steward | Lock before edit? |
|-------------|---------|-------------------|
| `components/ui/**` | Agent 1B (a11y) | **Yes** |
| `Layout.tsx`, `TitleBar.tsx`, `App.tsx` | Agent 1B | **Yes** |
| `PageHeader.tsx`, `EmptyState.tsx`, `LoadingPage.tsx` | Per iteration | **Yes** |
| Global CSS, `lib/pagination.ts` | Per iteration | **Yes** |
| `components/catalog-builder/**` | Agent 1A | No |
...
| **Agent 1B** | Labels, accessibility, visual clarity, UX polish |
| **Agent 1A** | **No role** (unless user reclassifies page) |
...
### Implementation record (Agent 1B)
...
**Agent 1A smoke (post Theme v2):** **PASS** — QA Stress Catalog; export preflight modal; preview iframe white; no builder regressions.
...
| Agent 1A UI | Complete — General → Opciones del PDF checkbox |

Registry: [CATALOG_PRESENTATION_BACKLOG.md](./CATALOG_PRESENTATION_BACKLOG.md)

### Presentation — catalogue (Agent 1A / 6)
...
## VARIANT-REPRESENTATION-1 — Frontend (Agent 1B)
...
| **Agent 1B** | **Idle** | ProductDetail v2 QA **PAUSED** |
| **Agent 1A** | **Idle** | Products smoke Wave 2 only |
| **Agent 4 / 6** | **Idle** | Paused tracks |

---

## ChatGPT chat (Agent 1B)

Use the dedicated **App-Wide Accessible UX/UI** ChatGPT chat (separate from Agent 1A catalogue chat). Agent 3 receives summaries; Agent 1B chat does not author API contracts or edit `api.ts`.
